...
然后添加 LSTM 层，这里我们使用了两层 LSTM 和 dropout 层，dropout 层可以防止过拟合。也可以适当加入卷积层，以提高模型的性能。
...
==========================================================================================
embedding_Embedding1 (Embed [[null,20]]               [null,20,16]              317280
__________________________________________________________________________________________
conv1d_Conv1D1 (Conv1D)     [[null,20,16]]            [null,18,24]              1176
__________________________________________________________________________________________
max_pooling1d_MaxPooling1D1 [[null,18,24]]            [null,9,24]               0
__________________________________________________________________________________________
lstm_LSTM1 (LSTM)           [[null,9,24]]             [null,9,24]               4704
__________________________________________________________________________________________
dropout_Dropout1 (Dropout)  [[null,9,24]]             [null,9,24]               0
__________________________________________________________________________________________
lstm_LSTM2 (LSTM)           [[null,9,24]]             [null,24]                 4704
__________________________________________________________________________________________
dense_Dense1 (Dense)        [[null,24]]               [null,2]                  50
==========================================================================================
Total params: 327914
Trainable params: 327914
Non-trainable params: 0
__________________________________________________________________________________________
```

可见，该模型有 327914 个参数，属于中等规模的模型。

接下来就是训练模型了，我们将预处理好的数据输入到模型中进行训练，由于 nodejs 以及性能问题的限制，为防止内存溢出，我们取了25000条数据进行训练，并使用300条数据进行测试。

```
__________________________________________________________________________________________
Epoch 1 / 50
eta=0.0 ================================================================>
64001ms 2462us/step - acc=0.776 loss=0.480
currectCount: 251, wrongCount: 49， accuracy: 0.8367
Epoch 2 / 50
eta=0.0 ================================================================>
66379ms 2553us/step - acc=0.870 loss=0.318
currectCount: 253, wrongCount: 47， accuracy: 0.8433
Epoch 3 / 50
eta=0.0 ================================================================>
63581ms 2446us/step - acc=0.918 loss=0.217
currectCount: 239, wrongCount: 61， accuracy: 0.7967
Epoch 4 / 50
eta=0.0 ================================================================>
64914ms 2497us/step - acc=0.948 loss=0.144
currectCount: 243, wrongCount: 57， accuracy: 0.81
Epoch 5 / 50
eta=0.0 ================================================================>
66663ms 2564us/step - acc=0.965 loss=0.101
currectCount: 250, wrongCount: 50， accuracy: 0.8333
```

可以看到，在第五轮训练时，模型在训练集上的准确率达到了 96.2%，在测试集上的准确率达到了 83%。效果可以接受。

在仅使用 LSTM 层，而不添加 CNN 层的情况下，测试集的预测准确率会高出3个百分点左右，但是训练时间会增加。

在仅使用 CNN 层，而不添加 LSTM 层的情况下，测试集的预测准确率会低8个百分点左右。
